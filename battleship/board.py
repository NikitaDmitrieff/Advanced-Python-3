...
class Board:
    ...
    def are_ships_within_bounds(self):
        """ Checks whether all ships occupy valid cells within the board.
        ...
        are_ships_within_bounds = True

        for ship in self.ships:

            if (1 <= ship.x_start <= self.width and
                    1 <= ship.x_end <= self.width and
                    1 <= ship.y_start <= self.height and
                    1 <= ship.y_end <= self.height):
                pass

            else:
                are_ships_within_bounds = False
                break

        return are_ships_within_bounds
    ...
    def have_all_ships_sunk(self):
        ...
        Returns:
            bool : return True if all ships on the board have sunk.
               return False otherwise.
        """
        have_all_ships_sunk = True

        for ship in self.ships:
            if not ship.has_sunk():
                have_all_ships_sunk = False  # Ship is still standing
                break
            else:
                pass

        return have_all_ships_sunk
    ...
    def is_attacked_at(self, cell):
        ...
        for ship in self.ships:
            if cell in ship.cells:  # Attack hit a ship
