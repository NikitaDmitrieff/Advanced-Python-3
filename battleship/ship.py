...
class Ship:
    ...
    def __init__(self, start, end, should_validate=True):
        ...
        self.x_start = min(self.x_start, self.x_end)
        self.x_end = max(self.x_start, self.x_end)
        
        # make y_start on top and y_end on bottom
        self.y_start = min(self.y_start, self.y_end)
        self.y_end = max(self.y_start, self.y_end)
